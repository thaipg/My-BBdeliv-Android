...
import dsc.vn.mybbdeliv.Model.Hub
...
import kotlinx.android.synthetic.main.content_output_warehouse_transfer.*
...
class OutputWarehouseTransferActivity : BaseActivity() , View.OnClickListener {

    private var allShipment: MutableList<Shipment> = mutableListOf()

    private var resultListHub:MutableList<Hub> = mutableListOf()
    private var resultListEmp:MutableList<Item> = mutableListOf()
    private var resultListTruck:MutableList<Truck> = mutableListOf()

    private var adapterHub: ArrayAdapter<String>? = null
    ...
    private var selectHub: Hub? = null
    private var selectEmp: Item? = null
    private var selectTransportType: Item? = null
    private var selectTruck: Truck? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        setContentView(R.layout.activity_output_warehouse_transfer)
    ...
    private fun prepareUI() {
        ...
        btTransportType.setOnClickListener(this)
        btnUpdate.setOnClickListener(this)
        txtPostOffice.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if (s != null) {
                    if (s.length >= 2) {
                        WarehouseProcess().getHubSearchByValue(s.toString()) {
                            Log.v("Hub :", it)
                            // If the response is JSONObject instead of expected JSONArray
                            resultListHub = Hub.Deserializer().deserialize(it)!!
                            setupAutocompleteViewHub()
                        }
                    }
                }
            }
        ...
        txtPostOffice.onItemClickListener = AdapterView.OnItemClickListener { parent, view, position, id ->
            selectHub = resultListHub[position]
            WarehouseProcess().getListGoodsTransferNew(selectHub!!.id.toString())
            {
                Log.v("BK :", it)
                if (it == "[]")
                {
                    WarehouseProcess().listGoodsCreateTransfer(selectHub!!.id.toString())
                    {
                        Log.v("BK : ", it);
    ...
    private fun setupAutocompleteViewHub() {
        // Get the string array
        val pickHub = java.util.ArrayList<Hub>()
        val pickHubName = java.util.ArrayList<String>()
        // Create the adapter and set it to the AutoCompleteTextView
        adapterHub = ArrayAdapter(applicationContext, android.R.layout.simple_spinner_dropdown_item, pickHubName)
        for (r in resultListHub) {
            pickHub.add(r)
            pickHubName.add(r.code + " - " + r.name)
        }
        txtPostOffice.setAdapter(adapterHub)
        txtPostOffice.setTextColor(Color.BLACK)
        txtPostOffice.setHintTextColor(Color.LTGRAY)
        txtPostOffice.highlightColor = Color.WHITE
        txtPostOffice.setLinkTextColor(Color.WHITE)
        txtPostOffice.showDropDown()
    }
        ...
        if (v != null) {
            when {
                v.id == imageView_scan_camera.id -> {
                    if (selectListGoods != null && selectHub != null) {
                        getBarcodeScan()
                    } else {
                        ToastUtils.error(this, "Chưa chọn bưu cục trung chuyển đến !")
                    }
                }
                v.id == imageView_shipment_plus.id -> {
                    if (selectListGoods != null && selectHub != null) {
                        ShipmentProcess().issueTransfer(this, selectListGoods!!.id, selectHub!!.id, txtShipmentCode.text.toString()) {
                ...
                v.id == btTransportType.id -> {
                    WarehouseProcess().getTransportType(this) {
                        val allTransportType = Item.Deserializer().deserialize(it)!!
                        val pickTransportType: ArrayList<Item> = ArrayList()
                        val pickTransportTypeName: ArrayList<String> = ArrayList()
                        allTransportType.forEach { r ->
                            pickTransportType.add(r)
                            pickTransportTypeName.add(r.name)
                        }

                        val arrayAdapter = ArrayAdapter<String>(
                                this, // Context
                                android.R.layout.simple_list_item_single_choice, // Layout
                                pickTransportTypeName // List
                        )

                        val builder = AlertDialog.Builder(this)
                        builder.setTitle("Chọn hình thức trung chuyển")
                        builder.setSingleChoiceItems(
                            ...
                            if (pickTransportType[i].id == 1)
                            {
                                txtTruck.setText("")
                                txtTruck.visibility = View.VISIBLE
                                txtFeeRent.setText("")
                                txtFeeRent.visibility = View.GONE
                            }
                            else
                            ...
                            selectTransportType = pickTransportType[i]
                            btTransportType.setText(pickTransportType[i].name)
                            v.dismiss()
                ...
                v.id == btnUpdate.id -> {
                    if (selectListGoods != null && selectHub != null) {
                        if (selectEmp != null)
                        {
                            selectListGoods!!.listGoodsTypeId = 8
                            selectListGoods!!.empId = selectEmp!!.id
                            if (selectTransportType != null)
                            {
                                selectListGoods!!.transportTypeId = selectTransportType!!.id
                                if (selectTransportType!!.id == 1)
                                {
                                    if (selectTruck != null)
                                    {
                                        selectListGoods!!.truckId = selectTruck!!.id
                                        if (selectTruck!!.truckOwnershipId == 2)
                                        {
                                    ...
                                    {
                                        ToastUtils.error(this, "Chưa chọn xe trung chuyển !")
                                        return
                                    }
                                }
                                else
                            ...
                            {
                                ToastUtils.error(this, "Chưa chọn hình thức trung chuyển !")
                                return
                            }
                        }
                        else
    ...
    private fun completeListGoods() {
        ...
        WarehouseProcess().blockListGoods(this, json) {
            Log.v("Result :", it)
            clearAll()
            ToastUtils.success(this, "Xuất kho trung chuyển thành công!")
        }
    }
    ...
    private fun clearAll() {
        ...
        txtBKCode.setText("")
        txtEmployee.setText("")
        btTransportType.setText("Chọn hình thức trung chuyển")
        txtTruck.setText("")
        txtTruck.visibility = View.GONE
        txtFeeRent.setText("")
        txtFeeRent.visibility = View.GONE
        ...
        if (resultCode == RESULT_OK && data != null) {
            try {
                if (requestCode == ZXingUtils.REQUEST_CODE) {
                    ...
                    if (selectListGoods != null && selectHub != null) {
                        ShipmentProcess().issueTransfer(this, selectListGoods!!.id, selectHub!!.id, result.contents) {
    ...
    private fun setupUI(view: View) {
        ...
        if (view !is EditText) {
            view.setOnTouchListener { v, event ->
                view.clearFocus()
                hideSoftKeyboard(this@OutputWarehouseTransferActivity)
                false
            }
        }
