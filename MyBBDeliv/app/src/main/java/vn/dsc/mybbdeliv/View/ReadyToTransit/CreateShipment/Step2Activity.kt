...
import kotlinx.android.synthetic.main.content_step2.*
import java.util.*

class Step2Activity : BaseSubActivity() , View.OnClickListener {
    ...
    private fun bindData()
    {
        ...
        {
            txtName.setText(shipment.receiverName)
            txtPhone.setText(shipment.receiverPhone)
            txtAddress.text = shipment.shippingAddress
            txtAddressNote.setText(shipment.addressNoteTo)
            txtCompany.setText(shipment.companyTo)

            if (shipment.toHubId != null) {
                val newHub = Hub()
                newHub.id = shipment.toHubId!!
                hub = newHub
            }

            if (shipment.toWard != null) {
                ward = shipment.toWard
                btWard.text = ward?.name
                btWard.setTextColor(Color.parseColor("#000000"))

                if (shipment.toWard!!.district != null) {
                    district = shipment.toWard!!.district
                    btDistrict.text = district?.name
                    btDistrict.setTextColor(Color.parseColor("#000000"))

                    if (shipment.toWard!!.district!!.province != null) {
                        province = shipment.toWard!!.district!!.province
            ...
            builder.setSingleChoiceItems(
                ...
                GeneralProcess().getHubByWardId(this, ward!!.id.toString(), {
                    if (!it.isNullOrEmpty()) {
                        ...
                        {
                            ToastUtils.warning(this, "Khu vực này chưa có trạm  hàng !")
                        }
                    }
                    else
                    {
                        ToastUtils.warning(this, "Không hỗ trợ nhận hàng tại địa điểm này !")
                    }
                })
                v.dismiss()
        ...
        if (requestCode == 9999 || requestCode == 9998) {
            if (resultCode == RESULT_OK) {
                ...
                if (requestCode == 9999) {
                    ...
                    if (address != null) {
                        shipment.latTo = address.latitude
                        shipment.lngTo = address.longitude
                                        ...
                                        if (!it.isNullOrEmpty()) {
                                            ...
                                            GeneralProcess().getHubByWardId(this, ward!!.id.toString(), {
                                                ...
                                                if (!it.isNullOrEmpty()) {
                                                    ...
                                                    {
                                                        ToastUtils.warning(this, "Khu vực này chưa có trạm nhận hàng !")
                                                    }
                                                }
                                                else
                                                {
    ...
    private fun nextStep() {
        ...
        if (btWard.isNotEmptySelector(this,"Không hỗ trợ giao hàng tại địa điểm này !",hub))
        {
            return
        }

        shipment.receiverName = txtName.text.toString()
        shipment.receiverPhone = txtPhone.text.toString()
        shipment.shippingAddress = txtAddress.text.toString()
        shipment.addressNoteTo = txtAddressNote.text.toString()
        shipment.companyTo = txtCompany.text.toString()

        shipment.toProvinceId = province!!.id
        shipment.toDistrictId = district!!.id
        shipment.toWardId = ward!!.id
        shipment.toHubId = hub!!.id
    ...
    private fun setupUI(view: View) {
        ...
        if (view !is EditText) {
            view.setOnTouchListener { v, event ->
                view.clearFocus()
                hideSoftKeyboard(this@Step2Activity)
                false
            }
        }
