...
import kotlinx.android.synthetic.main.content_delivery_fail.*
...
import dsc.vn.mybbdeliv.R.layout.activity_delivery_fail
import dsc.vn.mybbdeliv.Service.LocationTrackingService
import dsc.vn.mybbdeliv.Utils.ToastUtils
import java.util.*

class DeliveryFailActivity : BaseSubActivity() , View.OnClickListener{
    ...
    override val layoutResourceId: Int
        get() {
            return activity_delivery_fail
        }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_delivery_fail)
        prepareUI()
        bindData()
    }
        ...
        if (v != null) {
            if (v.id == btnUpdate.id)
            {
                ...
                for (item in selectedItems) {
                    ...
                    {
                        item.shipmentStatusId = 13
                    }
                    //Trả hàng thành công
                    else if (item.shipmentStatusId == 31)
                    {
                        item.shipmentStatusId = 28
                    }
    ...
    private fun setupUI(view: View) {
        ...
        if (view !is EditText) {
            view.setOnTouchListener { v, event ->
                view.clearFocus()
                hideSoftKeyboard(this@DeliveryFailActivity)
                false
            }
        }
