package dsc.vn.mybbdeliv.View.WaitingReceive
...
import kotlinx.android.synthetic.main.content_waiting_receive.*
import java.util.*


class WaitingReceiveActivity : BaseActivity() , AdapterView.OnItemClickListener , View.OnClickListener {
    ...
    private fun prepareUI() {
        listView = findViewById<ListView>(listView_waiting_receive.id)
        listView.onItemClickListener = this
        listView.choiceMode = ListView.CHOICE_MODE_MULTIPLE
        btPickup.setOnClickListener(this)

        listView.isTextFilterEnabled = true
        searchViewContentWaitingReceive.setIconifiedByDefault(false)
        searchViewContentWaitingReceive.isSubmitButtonEnabled = false
        searchViewContentWaitingReceive.queryHint = "Nhập mã vận đơn hoặc bảng kê phát"
        searchViewContentWaitingReceive.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
        ...
        searchViewContentWaitingReceive.post { searchViewContentWaitingReceive.clearFocus() }
    }

    @SuppressLint("ResourceAsColor")
    private fun bindData() {
        allShipment.clear()
        ShipmentProcess().getShipmentReadyToTransfer (this) {
            for (s in Shipment.Deserializer().deserialize(it)!!) {
                allShipment.add(s)
            }
            ShipmentProcess().getShipmentReadyToTransferReturn (this) {
                for (s in Shipment.Deserializer().deserialize(it)!!) {
                    allShipment.add(s)
                }
        ...
        if (v != null) {
            if (v.id == btPickup.id) {
                for (item in selectedItems) {
                    // Trung chuyển
                    if (item.shipmentStatusId == 49)
                    {
                        item.shipmentStatusId = 8
                    }
                    // Trung chuyển trả hàng
                    if (item.shipmentStatusId == 50)
                    {
                        item.shipmentStatusId = 37
                    }
                    ...
                    ShipmentProcess().updateShipment(this,item, {
                        ...
                        {
                            ToastUtils.success(applicationContext, "Đã xác nhận trung chuyển!")
